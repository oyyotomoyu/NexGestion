...
const mockPermissions:Permission[]=["users.read","users.manage","roles.read","roles.manage","roles.assign","groups.read","groups.manage","groups.assign","permissions.read","permissions.assign","permissions.manage"].map((key)=>({id:key,permission_key:key,module:key.split(".")[0],description:null}));
...
export function setGroupPermission(groupId:string,permissionId:string,grant:boolean){return request<void>(`/api/groups/${encodeURIComponent(groupId)}/permissions/${encodeURIComponent(permissionId)}`,{method:grant?"PUT":"DELETE"});}
